...
class AWSTranscriber:
    ...
    def _run_transcription(self, transcribe, s3_key: str) -> list[TranscriptSegment]:
        ...
        try:
            ...
            with httpx.Client(timeout=30) as client:
                r = client.get(result_uri)
                r.raise_for_status()
                data = json.loads(r.text)
        except Exception as e:
            raise RuntimeError(f"Failed to fetch transcript result: {e}") from e
    ...
    def _parse_results(self, data: dict) -> list[TranscriptSegment]:
        ...
        try:
            ...
            raise RuntimeError(f"Unexpected transcript response structure: {list(data.keys())}")
        if not items:
            return []
